def hati():
  ...
  file1 = open("../silsilah-hati/index.md", "w")
  file1.write('''<script
            async
            defer
...
v.mozPreservesPitch = v.webkitPreservesPitch = v.preservePitch = !p;
  ...
  for file in files:
      judul = '-   {}'.format(file.replace('.mp3', '').replace("Tawadhu_", "Tawadhu'"))
      judul = judul + "\n\n"
      judul = judul + '    <audio controls preload="none">'
      judul = judul + "\n" + "\n"
      judul = judul + '    <source src="/kajian-ufa/silsilah-hati/Audio%20-%20Silsilah%20Hati/{}" type="audio/mpeg">'.format(file.replace(" ", "%20"))
      judul = judul + "\n"
      judul = judul + "    </audio>"
      judul = judul + "\n\n"
      print(judul)
      file1 = open("../silsilah-hati/index.md", "a")
      file1.write(judul)
      file1.close()
